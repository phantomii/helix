//+------------------------------------------------------------------|
//|                                                            JAson |
//|                            Copyright © 2006-2016  Alexey Sergeev |
//|    This software is licensed under the MIT https://goo.gl/eyJgHe |
//+------------------------------------------------------------------+
#property copyright "Copyright © 2006-2016  Alexey Sergeev"
...
public:
	...
	virtual void FromStr(enJAType t, string a)
	{
		...
		case jtSTR: m_sv=Unescape(a); m_type=m_sv?jtSTR:jtNULL; m_iv=StringToInteger(m_sv); m_dv=StringToDouble(m_sv); m_bv=m_sv!=NULL; break;
		}
	}
	virtual string GetStr(char& js[], int i, int slen) { char cc[]; ArrayCopy(cc, js, 0, i, slen); return CharArrayToString(cc, 0, WHOLE_ARRAY, CJAVal::code_page); }
...
bool CJAVal::Deserialize(char& js[], int slen, int &i)
{
	...
	for (; i<slen; i++)
	{
		...
		case '0': case '1': case '2': case '3': case '4': case '5': case '6': case '7': case '8': case '9': case '-': case '+': case '.': // начало числа
		{
			...
			else { m_type=jtINT; m_iv=StringToInteger(m_sv); m_dv=(double)m_iv; } // уточнии тип значения
